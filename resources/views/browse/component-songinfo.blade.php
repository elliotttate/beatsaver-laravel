...
<table class="table" style="table-layout:fixed;">
    <tr>
        <th rowspan="5" style="width: 15%;"><a href="{{ route('browse.detail',['id' => $downloadKey]) }}"><img src="img/{{ $cover }}.{{ $coverMime }}" alt="{{ $name }}"
    ...
    <tr>
        <td>Downloads: {{ $downloadCount }} || Finished: {{ $playedCount }}
            <br>
            @auth
                <a href="{{ route('votes.down', ['id' => $downloadKey]) }}" class="pull-right">
                    <button type="button" class="btn btn-default">
                        <span class="glyphicon glyphicon-thumbs-down" aria-hidden="true"></span> Down {{ $downvotes }}
                    </button>
                </a>
                <a href="{{ route('votes.up', ['id' => $downloadKey]) }}" class="pull-right">
                    <button type="button" class="btn btn-default">
                ...
                <br><br>
            @endauth
            <p><a class="btn btn-default" href="{{ route('download', ['id' => $downloadKey]) }}" role="button">Download File</a></p>
        </td>
    </tr>
</table>
